...
def make_parser():
    ...
    parser.add_argument('--refinement_method',
                        ...
                        default='sampling')
    parser.add_argument('--refine_steps', type=int, default=25)#ori 25
    ...
    parser.add_argument('--target_pc_size', type=int, default=1024)#ori 1024
        ...
        while True:
                ...
                for npy_file in glob.glob(os.path.join(args.npy_folder, '*.npy')):
                    ...
                    data = np.load(npy_file, allow_pickle=True,
                                encoding="latin1")
                    data2 = np.load('/home/po/TM5/UnseenObjectClustering/BG.npy', allow_pickle=True,
                                encoding="latin1").item()
                    print(data)
                    pc = data2['xyz_img']
                    pc_colors = data2['color']
                    # pc_colors = np.tile(pc_colors,[1,1,1,1,1,1])
                    # depth = data['depth']
                    # image = data['image']
                    ...
                    draw_scene(
                        pc.reshape(-1,3),
                        pc_color=pc_colors.reshape(-1,3),
                        grasps=generated_grasps,
                        grasp_scores=generated_scores,
                    )
                    print('close the window to continue to next object . . .')
